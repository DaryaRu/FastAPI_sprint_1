...
@router.get("/", response_model=list[Genre], summary="Получить список всех жанров")
async def genre_list(
    sort: str | None = Query(
        None,
        description="Сортировка по имени по алфавиту, в прямом и обратном порядке(name и -name)",
    ),
...
@router.get("/{genre_uuid}/", response_model=Genre, summary="Получить жанр по UUID")
