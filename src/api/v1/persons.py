...
@router.get(
    "/search", response_model=list[Person], summary="Полнотекстовый поиск по персонам"
)
...
@router.get(
    "/{person_uuid}/", response_model=Person, summary="Получить персону по UUID"
)
...
@router.get("/", response_model=list[Person], summary="Получить список всех персон")
