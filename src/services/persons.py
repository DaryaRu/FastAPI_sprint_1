...
class PersonService:
    ...
    def __init__(self, elastic: AsyncElasticsearch):
        ...
        self.person_repo = PersonsRepository(elastic_client=elastic, index="persons")
        self.movie_repo = FilmRepository(elastic_client=elastic, index="movies")
    ...
    async def get_list(
        ...
        if query:
            ...
            docs_sources = await self.person_repo.get_filtered(
                page_size=page_size, page_number=page_number, query={"match_all": {}}
            )
        return [PersonModel(**source) for source in docs_sources]

    async def get_person_films(self, person_uuid: UUID) -> Optional[List[FilmShort]]:
            ...
            "bool": {
                "should": [
                    ...
                    },
                    {
                        "nested": {
                            "path": "writers",
                            "query": {"term": {"writers.id": str(person_uuid)}},
                        }
                    },
                    {
                        "nested": {
                            "path": "directors",
                            "query": {"term": {"directors.id": str(person_uuid)}},
                        }
                    },
                ]
            }
